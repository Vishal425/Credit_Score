...
def sql_value(val1):
    ...
    print("Accuray of  logistic regression", accuracy_score(y_test,y_pred_log))
    ...
    npa = pd.DataFrame(reg.predict(X1), columns = ['NPA'])
    npa =npa.replace({'NPA': {0: 'YES', 1: 'NO'}})
    df2 = pd.concat([X1,credit_score,npa],axis = 1)
    df2['CREDIT_SCORE']=round(df2['CREDIT_SCORE'].mean())
    ...
    # #import pickle
    # #pipline_from_pickle = pickle.load(open(r'D:\Python_Project\py\credit_analysis\pkl_file\loanscore12_1.pkl','rb'))
    # credit_score = X_test_woe_transformed.dot(pipline_from_pickle)#
    # #print(credit_score)
    #credit_numbers = credit_score

    # model = pickle.load(open(r'D:\Python_Project\py\credit_analysis\pkl_file\npamodel_1.pkl','rb'))
    # print(model)

    #NPA = model.predict(X1)
    


    # credit_score.columns = ['CREDIT_SCORE']#
    # npa = pd.DataFrame(model.predict(X1), columns = ['NPA'])
    # npa =npa.replace({'NPA': {0: 'YES', 1: 'NO'}})
    # df2 = pd.concat([df2,credit_score,npa],axis = 1)
    # df2['CREDIT_SCORE']=round(df2['CREDIT_SCORE'].mean())
    # print('Predicted DataFrame',df2.to_csv('predicted_data.csv',index=False))
